...
translate = boto3.client(service_name='translate', aws_access_key_id="" , aws_secret_access_key="")
    ...
    for slide in presentation.slides:
        ...
        if slide.has_notes_slide:
            ...
            if len(text_frame.text) > 0:
                ...
                try:
                    print("*************");
                    print("Translate Notes Slide");
                    print("[FROM] : " + text_frame.text);
                    response = translate.translate_text(
                            Text=text_frame.text,
                            SourceLanguageCode=source_language_code,
                            TargetLanguageCode=target_language_code,
                            TerminologyNames=terminology_names)
                    print("[TO]  " + response.get('TranslatedText'));                            
